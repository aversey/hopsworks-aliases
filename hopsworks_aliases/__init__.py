...
class HopsworksAliasesError(Exception):
    ...
    Ideally, this exception should never happen, as it means misconfiguration of aliases.
    """
...
def public(
    *paths: str,
    ...
    """Make a function or class publicly available via an alias.

    Parameters:
        paths: The import paths under which the entity is publicly available; effectively results in generation of aliases in all of the paths for the entity.
    ...
    def deprecate(symbol):
        ...
        if inspect.isfunction(symbol):
            ...
            def deprecated_f(*args, **kwargs):
                ...
                if not caller or (
                    not caller.__name__.startswith("hopsworks")
                    and not caller.__name__.startswith("hsfs")
                    and not caller.__name__.startswith("hsml")
                ):
                    warnings.warn(
